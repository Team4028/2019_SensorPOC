...
import static frc.robot.sensors.revSrc.VL53L0X.Register.DYNAMIC_SPAD_NUM_REQUESTED_REF_SPAD;
import static frc.robot.sensors.revSrc.VL53L0X.Register.DYNAMIC_SPAD_REF_EN_START_OFFSET;
import static frc.robot.sensors.revSrc.VL53L0X.Register.FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT;
import static frc.robot.sensors.revSrc.VL53L0X.Register.FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI;
import static frc.robot.sensors.revSrc.VL53L0X.Register.FINAL_RANGE_CONFIG_VCSEL_PERIOD;
import static frc.robot.sensors.revSrc.VL53L0X.Register.GLOBAL_CONFIG_REF_EN_START_SELECT;
import static frc.robot.sensors.revSrc.VL53L0X.Register.GLOBAL_CONFIG_SPAD_ENABLES_REF_0;
import static frc.robot.sensors.revSrc.VL53L0X.Register.GPIO_HV_MUX_ACTIVE_HIGH;
import static frc.robot.sensors.revSrc.VL53L0X.Register.MSRC_CONFIG_CONTROL;
import static frc.robot.sensors.revSrc.VL53L0X.Register.MSRC_CONFIG_TIMEOUT_MACROP;
import static frc.robot.sensors.revSrc.VL53L0X.Register.OSC_CALIBRATE_VAL;
import static frc.robot.sensors.revSrc.VL53L0X.Register.PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI;
import static frc.robot.sensors.revSrc.VL53L0X.Register.PRE_RANGE_CONFIG_VCSEL_PERIOD;
import static frc.robot.sensors.revSrc.VL53L0X.Register.RESULT_INTERRUPT_STATUS;
import static frc.robot.sensors.revSrc.VL53L0X.Register.RESULT_RANGE_STATUS;
import static frc.robot.sensors.revSrc.VL53L0X.Register.SYSRANGE_START;
import static frc.robot.sensors.revSrc.VL53L0X.Register.SYSTEM_INTERMEASUREMENT_PERIOD;
import static frc.robot.sensors.revSrc.VL53L0X.Register.SYSTEM_INTERRUPT_CLEAR;
import static frc.robot.sensors.revSrc.VL53L0X.Register.SYSTEM_INTERRUPT_CONFIG_GPIO;
import static frc.robot.sensors.revSrc.VL53L0X.Register.SYSTEM_SEQUENCE_CONFIG;
import static frc.robot.sensors.revSrc.VL53L0X.Register.VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV;
...
public class VL53L0X {
    ...
    public VL53L0X() {
        /*
          MSBit                        LSBit
            0   1   0   1   0   0   1   R/W     The first byte contains the device address (0x52) and also specifies the data direction
            0   1   0   1   0   0   1   0       0x52    Master Read from Slave      
            0   1   0   1   0   0   1   1       0x53    Master Write from Slave     

            0   1   0   1   0   0   1           0x29  
        */

        // 7 bit adddressing
        this(I2C.Port.kOnboard, 0x52 >>> 1);
    ...
    public boolean doInitialize() {
        ...
        boolean isSensorPresent = true;

        System.out.format("Checking to see if it's really a VL53L0X sensor...");

        bVal = readRegister(0xC0);
        isSensorPresent = isSensorPresent && (bVal == 0xEE);
        System.out.format("Reg 0xC0 = %x (should be 0xEE)", bVal);

        bVal = readRegister(0xC1);
        isSensorPresent = isSensorPresent && (bVal == 0xAA);
        System.out.format("Reg 0xC1 = %x (should be 0xAA)", bVal);

        bVal = readRegister(0xC2);
        isSensorPresent = isSensorPresent && (bVal == 0x10);
        System.out.format("Reg 0xC2 = %x (should be 0x10)", bVal);

        bVal = readRegister(0x51);
        isSensorPresent = isSensorPresent && (bVal == 0x0099);
        System.out.format("Reg 0x51 = %x (should be 0x0099)", bVal);

        bVal = readRegister(0x61);
        isSensorPresent = isSensorPresent && (bVal == 0x0000);
        System.out.format("Reg 0x61 = %x (should be 0x0000)", bVal);

        // initialize the sensor.
        // if you specify false as the bUse2v8 argument, the sensor will operate in 1.8V mode.
        return initVL53L0X(false) && isSensorPresent;
    }
    ...
    private boolean initVL53L0X(boolean bUse2v8) {
        ...
        if (bUse2v8) {
            ...
            writeRegister(VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV,
                    (byte)(readRegister(VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV) | 0x01));
        }
        ...
        stop_variable = this.readRegister(0x91);
        ...
        writeRegister(MSRC_CONFIG_CONTROL, (byte)(readRegister(MSRC_CONFIG_CONTROL) | 0x12));

        // debug.
        // check rate limit before we set it.
        //RobotLog.dd(MYTAG, "initial sig rate lim (MCPS) %.06f", getSignalRateLimit());
        System.out.format("initial sig rate lim (MCPS) %.06f", getSignalRateLimit());
        ...
        byte ref_spad_map[];
        //ref_spad_map = this.deviceClient.read(GLOBAL_CONFIG_SPAD_ENABLES_REF_0.bVal, 6);
        ref_spad_map = this.readRegisters(GLOBAL_CONFIG_SPAD_ENABLES_REF_0.bVal, 6);

        // -- VL53L0X_set_reference_spads() begin (assume NVM values are valid)

        writeRegister(0xFF, 0x01);
        writeRegister(DYNAMIC_SPAD_REF_EN_START_OFFSET.bVal, 0x00);
        writeRegister(DYNAMIC_SPAD_NUM_REQUESTED_REF_SPAD.bVal, 0x2C);
        writeRegister(0xFF, 0x00);
        writeRegister(GLOBAL_CONFIG_REF_EN_START_SELECT.bVal, 0xB4);
        ...
        writeRegisters(GLOBAL_CONFIG_SPAD_ENABLES_REF_0.bVal, ref_spad_map);
        ...
        writeRegister(SYSTEM_INTERRUPT_CONFIG_GPIO.bVal, 0x04);
        writeRegister(GPIO_HV_MUX_ACTIVE_HIGH.bVal, readRegister(GPIO_HV_MUX_ACTIVE_HIGH) & ~0x10); // active low
        writeRegister(SYSTEM_INTERRUPT_CLEAR.bVal, 0x01);
        ...
        writeRegister(SYSTEM_SEQUENCE_CONFIG.bVal, 0xE8);
        ...
        writeRegister(SYSTEM_SEQUENCE_CONFIG.bVal, 0x01);
        ...
        writeRegister(SYSTEM_SEQUENCE_CONFIG.bVal, 0x02);
    ...
    private boolean getSpadInfo() {
        ...
        writeRegister(0x83, (byte) (this.readRegister(0x83) | 0x04));
    ...
    protected void getSequenceStepEnables(SequenceStepEnables enables) {
        int sequence_config = readRegister(SYSTEM_SEQUENCE_CONFIG);
    ...
    protected void getSequenceStepTimeouts(SequenceStepEnables enables, SequenceStepTimeouts timeouts) {
        ...
        timeouts.msrc_dss_tcc_mclks = readRegister(MSRC_CONFIG_TIMEOUT_MACROP) + 1;
    ...
    protected int getVcselPulsePeriod(vcselPeriodType type) {
        if (type == VcselPeriodPreRange) {
            return decodeVcselPeriod(readRegister(PRE_RANGE_CONFIG_VCSEL_PERIOD));
        } else if (type == VcselPeriodFinalRange) {
            return decodeVcselPeriod(readRegister(FINAL_RANGE_CONFIG_VCSEL_PERIOD));
        } else {
            return 255;
        }
    }
    ...
    protected boolean performSingleRefCalibration(int vhv_init_byte) {
        writeRegister(SYSRANGE_START.bVal, 0x01 | vhv_init_byte); // VL53L0X_REG_SYSRANGE_MODE_START_STOP
        ...
        writeRegister(SYSRANGE_START.bVal, 0x00);
    ...
    protected void startContinuous(int period_ms) {
        ...
        if (period_ms != 0) {
            ...
            this.writeRegisters(SYSTEM_INTERMEASUREMENT_PERIOD.bVal, TypeConversion.intToByteArray(period_ms));

            // VL53L0X_SetInterMeasurementPeriodMilliSeconds() end

            writeRegister(SYSRANGE_START.bVal, 0x04); // VL53L0X_REG_SYSRANGE_MODE_TIMED
        } else {
            // continuous back-to-back mode
            writeRegister(SYSRANGE_START.bVal, 0x02); // VL53L0X_REG_SYSRANGE_MODE_BACKTOBACK
        }
    }
    ...
    protected void  stopContinuous() {
        writeRegister(SYSRANGE_START.bVal, 0x01); // VL53L0X_REG_SYSRANGE_MODE_SINGLESHOT
    ...
    protected int readRangeContinuousMillimeters() {
        ...
        while ((readRegister(RESULT_INTERRUPT_STATUS) & 0x07) == 0) {
        ...
        int range = (int)TypeConversion.byteArrayToShort(this.readRegisters(RESULT_RANGE_STATUS.bVal + 10, 2));
        writeRegister(SYSTEM_INTERRUPT_CLEAR.bVal, 0x01);
    ...
    // Read Methods
    // ==============================================================================================
    protected byte readRegister(Register reg) {
        return this.readRegister(reg.bVal);
    }

    private byte readRegister(int registerAddress) {
        _i2cBus.read(registerAddress, 1, _dataBuffer);
        byte bVal = _dataBuffer[0];

        return bVal;
    } 

    private byte[] readRegisters(int registerAddress, int count) {
        byte[] data = new byte[count];  
        _i2cBus.read(registerAddress, count, data);

        //_i2cBus.read(0xC0, count, _byteBuffer);
        //_byteBuffer.get(data);      
        // if sensor in little endian...swap bytes
        //ByteBuffer raw = ByteBuffer.allocate(2);
        //raw.order(ByteOrder.LITTLE_ENDIAN);
        //return raw.getShort(0);

        return data;
    } 

    protected short readShort(Register reg) {
        return TypeConversion.byteArrayToShort(readRegisters(reg.bVal, 2));
    }

    // ==============================================================================================
    // Write Methods
    // ==============================================================================================
    protected void writeRegister(Register reg, byte value) {
        this.writeRegister(reg.bVal, value);
    }

    private void writeRegister(int registerAddress, int data) {
        _i2cBus.write(registerAddress, data);
    }

    private void writeRegisters(int registerAddress, byte[] data) {
        byte[] buffer = new byte[data.length + 1];

        // add register address as 1st byte in array
        buffer[0] = (byte)registerAddress;

        System.arraycopy(data, 0, buffer, 1, data.length);

        _i2cBus.writeBulk(buffer);
    }

    protected void writeShort(final Register reg, short value) {
        writeRegisters(reg.bVal, TypeConversion.shortToByteArray(value));
    }
}
